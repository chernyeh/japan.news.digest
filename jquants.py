...
def load_3m_perf_from_github(repo: str, token: str = None) -> dict:
    ...
    """
    import csv, io, base64, gh_read

    status, raw = gh_read.raw_get(repo, "data/jquants_3m_perf.csv", token)
    if status != 200:
...
def load_perf_map_from_github(repo: str, token: str = None) -> tuple:
    ...
    status, _text = gh_read.raw_get(repo, "data/jquants_3m_perf.csv", token)
    if status != 200:
...
def load_prices_from_github(repo: str, token: str = None) -> dict:
    ...
    status, _text = gh_read.raw_get(repo, "data/jquants_prices_latest.csv", token)
    if status != 200:
        return {}
    r = type("R", (), {"text": _text})()
...
def compute_perf_map_inline(prices_map: dict, repo: str = None, token: str = None) -> tuple:
    ...
    def _load_csv_prices(path, headers=None):
        import gh_read
        status, text = gh_read.raw_get(repo, path, token, timeout=20)
        if status != 200:
            return {}
        prices = {}
    ...
    for period, (target_days, tolerance) in PERIODS.items():
        ...
        arch = _load_csv_prices(f"data/archive/{fname}")
        if not arch:
